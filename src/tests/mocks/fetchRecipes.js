...
const mockFetch = (url) => Promise.resolve({
  json: () => {
    ...
    if (url === 'https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Cocktail') return Promise.resolve(mockCategoriesDrinks);
    if (url === 'https://www.thecocktaildb.com/api/json/v1/1/search.php?s=') return Promise.resolve(mockDrinks);
    if (url === 'https://www.thecocktaildb.com/api/json/v1/1/list.php?c=list') return Promise.resolve(mockFilterByCategoryDrink);
  },
});
